...
Internally, the worker depends only on a 5-method `TTExecutionBackend` ABC (`new_request` / `extend` / `decode_step` / `free` / `reset_all`) via a `TT_EXECUTION_BACKENDS` registry keyed by env var `SGLANG_TT_EXECUTION_BACKEND`. P1 ships exactly one implementation (`TTTransformersExecutionBackend`); a `TTXLAExecutionBackend` slot is reserved for post-P1 to make tt-xla (Tenstorrent's general PyTorch/JAX frontend, deprecating tt-torch) a backend-swap rather than a Phase-F rewrite. See §3.2 invariant #7.
...
| 15 | `test/test_model_runner_init.py` | `TTModelRunner` stub asserts `device == "cpu"`, `attn_backend is None`, `sampler is None`, and `graph_runner is None` (Phase D.2 deliverable) | ~30 |
...
After the initial spec was approved, a follow-up amendment introduced the `TTExecutionBackend` ABC + `TT_EXECUTION_BACKENDS` registry (`tt_transformers` as P1's real impl, `tt_xla` as a registered placeholder for post-P1). The amendment passed through **8 rounds of independent fresh-eyes subagent review** between commits `6c6b032cf` (round 1 introduction) and the current HEAD. Findings per round:
...
- **Round 6** (review of round 5 → fixed in `4e0ea3b8b`): **1 CRITICAL + 3 IMPORTANT**. CRITICAL: F.3 lifecycle tests patched `create_tt_model` + `Generator` but not `ttnn` — the round-5 hoist + None sentinel meant non-TT-host test runs would AttributeError in `__init__` before reaching the mocked names; added `@patch(f"{_MODULE}.ttnn")` to both lifecycle tests. IMPORTANT: spec §7.2 / §7.3 still had three `wrapper.free` / `wrapper.reset_all` references that round-3 missed (exactly the regression pattern Appendix B warned about); Round-5 miscount (claimed 3 IMPORTANT, actually 2) corrected here; F.1 Step 5 Option B "fine for the F.1 prototype" recommendation gained a `python -O` caveat (asserts get stripped, so Option B fails silently under optimization).
- **Round 7** (review of round 6 → fixed in `e20d10982`): **0 CRITICAL + 3 IMPORTANT + 2 NIT**. Verdict: F.1 readiness **GREEN**. IMPORTANT: §0 Executive Summary still described the original monolithic black-box without mentioning the ABC + registry (cold readers got the wrong mental model) — added a second paragraph; Appendix B "4 rounds" / "5 rounds" count went stale after rounds 5+6 — updated and the takeaway rewritten. The parallel paragraph at the top of the plan (top-of-file **Architecture** paragraph, plan line 7) was updated for consistency.
- **Round 8** (review of round 7 → fixed in this commit): **0 CRITICAL + 4 IMPORTANT**. Verdict: F.1 readiness still **GREEN** — only doc-freshness gaps. IMPORTANT: Appendix B intro and takeaway both said "6 rounds" but enumerated 7 (R7's own update to the count was self-referentially stale); R7's bullet pointed to a non-existent "plan §A.0 Status" instead of the actual plan line 7 Architecture paragraph; `test_model_runner_init.py` (created in plan D.2 and committed in `757e05948`) was missing from spec §4.1 row table and Appendix A's `test/` tree; plan F.1 Step 5 Option A guard instruction said "before any use of the hoisted names" without explicitly noting that path validation (which uses no hoisted names and is asserted by `test_missing_model_path_raises`) must precede the ttnn-None guard so the FileNotFoundError path is reachable on CPU-only test hosts. All four fixed inline.

**Takeaway from 8 rounds**: every round caught issues the prior round missed; rounds 7-8 were the first two where the implementation was actually ready (only doc-freshness fixes remained — two consecutive rounds with 0 CRITICAL marks the convergence point). The repeating pattern through rounds 2-6 was *new code samples* (especially short pseudocode) introducing fresh bugs when fixing older ones — particularly around import semantics (lazy vs module-scope), enum-equality vs predicate methods, and stale identifier renames. Rounds 7-8 specifically exposed the doc-freshness failure mode of self-referential count updates: R7 updated "5 rounds → 6 rounds" *describing itself*, which immediately went stale; R8 catches the same trap. The discipline of running another fresh-eyes review after each fix round was load-bearing: skipping it would have shipped F.1 with at least one of `import ttnn` in worker code, missing `ForwardMode` import, `AttributeError: None.bfloat16` at the first dtype-map call, or a `RuntimeError` from Option A fired before the test-asserted `FileNotFoundError`.
