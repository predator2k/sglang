...
| A.1.a | `/home/mhnie/tt-xla/pjrt_implementation/src/api/module_builder/module_builder.cc` | Build a `TTArgumentTypeMap` and assign it to both `stablehlo_pipeline_options.argumentTypeMap` (StableHLO PM, around line 832; variable is `stablehlo_pipeline_options`) and `options.argumentTypeMap` (TTNN PM, around line 985; the C++ variable in that block is named `options`, of type `TTIRToTTNNCommonPipelineOptions`, NOT `ttnn_pipeline_options`). Pipeline-option fields already exist in `TTNNPipelines.h:319–336` and `StableHLOPipelines.h:47–61`. Plug-in source lives at `/home/mhnie/tt-xla/pjrt_implementation/{inc,src}/`. |
| A.2.a | `/home/mhnie/tt-mlir-sglang` (branch `tenstorrent-p1`) | new `lib/Dialect/TTNN/Transforms/TTNNFoldThroughAgnosticOps.cpp` + `Passes.td` registration + `lib/Dialect/TTNN/Pipelines/TTNNPipelines.cpp` insertion (see Phase 3 for exact insertion point) |
| A.3.a | `/home/mhnie/tt-mlir-sglang` | Either `lib/Conversion/StableHLOToTTIR/StableHLOToTTIRPass.cpp` OR a small new pass under `lib/Dialect/StableHLO/Transforms/` scheduled in `StableHLOPipelines.cpp` AFTER `createTTPopulateArgumentTypes` (so the explicit map from Phase 2 wins for mapped functions and auto-detect fills the rest; see §5 Phase 4 rationale). Note: `lib/Dialect/StableHLO/Transforms/AnalyzeMesh.cpp` *reads* `ArgumentTypeAttr` and emits errors (lines 120–132) when an arg isn't already annotated — it doesn't populate, so extending it would change its semantics, not "tweak a config". Don't go there. |
...
|---|---|---|---|---|
| 0.0 | Snapshot canonical `.so` SHAs as Phase 1 baseline | `docker exec tt-xla-eval bash -c 'sha256sum /tt-xla/third_party/tt-mlir/install/lib/*.so' > /tmp/canonical-lib-shas.txt` (see Phase 0.0 prose above for context) | `/tmp/canonical-lib-shas.txt` exists, contains ≥ 1 line | None — mandatory precondition for Phase 1 step 6 |
...
- Option (a) `ttmlir-opt --print-op-stats`: the binary lives at `/tt-xla/third_party/tt-mlir/src/tt-mlir/build/bin/ttmlir-opt` BEFORE Phase 1 (canonical clone; `build/bin/`, not `build/tools/` — verified empirically); after Phase 1 the fork's build at `/home/mhnie/tt-mlir-sglang/build/bin/ttmlir-opt` is the right one. Phase 0 runs pre-Phase-1, so use the canonical-clone path.
...
1. Edit `/home/mhnie/tt-xla/third_party/CMakeLists.txt`:
   - ExternalProject_Add block at lines 47–85:
     ...
     - LEAVE `INSTALL_COMMAND` in place, but **explicitly add `--prefix ${TTMLIR_INSTALL_PREFIX}` to the install invocation** (requires CMake ≥ 3.15, which the project already uses). The default install prefix cached in the fork's `build/CMakeCache.txt` is `/usr/local`, NOT `${TTMLIR_INSTALL_PREFIX}` — `build_and_install.sh:25–29` configures the fork without setting `-DCMAKE_INSTALL_PREFIX=${TTMLIR_INSTALL_PREFIX}`. Without overriding via `--prefix`, the install would silently land .so files in `/usr/local/lib`, leaving the canonical `${TTMLIR_INSTALL_PREFIX}/lib/*.so` files untouched. tt-xla's `file(GLOB TTMLIR_LIBRARIES "${TTMLIR_LIB_DIR}/*.so")` at line 101 would then link against the canonical artifacts, the Phase 1 SHA-diff would PASS (canonical .so's never changed), and Phase 2 onward would silently run against the wrong binary. The full INSTALL_COMMAND replacement (BOTH lines must include `--prefix`):
```
...
```
Alternative (less invasive of the CMakeLists.txt): hardcode `-DCMAKE_INSTALL_PREFIX=/home/mhnie/tt-xla/third_party/tt-mlir/install` into `build_and_install.sh:25–29` so the fork's CMakeCache stores the right prefix from the start. Note this is a literal path because the script (in tt-mlir-sglang) has no access to tt-xla's `${TTMLIR_INSTALL_PREFIX}` CMake variable.
...
2. Assign `stablehlo_pipeline_options.argumentTypeMap = classifyArgs(mlir_module);` AFTER line 832 (which already sets `stablehlo_pipeline_options.resultPresharded`) and immediately BEFORE the call to `createStableHLOPipeline(stablehlo_pipeline_pm, stablehlo_pipeline_options)` at line 833.
...
Per phase (Phase 1 onward — Phase 0 is investigation-only and has no bench/commit cadence): Tracy probe → server bench → commit. ~30 min per loop. Isolates which change moved which metric and keeps fixtures aligned with commits.
...
| Phase 1 `TT_METAL_RUNTIME_ROOT` could resolve to a deleted canonical path | Phase 1 leaves lines 36–39 unchanged (line 37's `if(NOT DEFINED ENV{TT_METAL_RUNTIME_ROOT})` guard skips the hardcode when the env var is already set). Phase 1 step 3 has build_and_install.sh export `TT_METAL_RUNTIME_ROOT=/home/mhnie/tt-mlir-sglang/third_party/tt-metal/src/tt-metal` before cmake runs, so the guard kicks in. |
...
- Phase 0: ~1.5 hours (0.0 ≤ 1 min single docker-exec sha256sum, 0.1 ≈ 30 min, 0.2 ≈ 45 min including `export_path` wire-up + Python-bool round-trip, 0.3 ≈ 15 min)
...
Total: ~9 hours minimum (Phase 0 + 1 + 2 + 5 = 1.5 + 3 + 4 + 0.5; no Phase 3/4), ~16 hours maximum (all phases = 1.5 + 3 + 4 + 5 + 2 + 0.5; Phase 3 is 5h per line 331).

## 9. Open items to confirm in writing-plans

- Naming for the new fold pass — current name `TTNNFoldThroughAgnosticOps` is informative but may be revised after reading existing TTNN pass-naming conventions in `lib/Dialect/TTNN/Transforms/Passes.h`.
