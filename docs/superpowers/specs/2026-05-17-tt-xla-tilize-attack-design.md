...
**4.4 IR dumping is a built-in compile_options field, not a research item.** `compile_options.h:121` defines `std::optional<std::string> export_path`. When set, `ModuleBuilder::printModule` (defined at `module_builder.cc:1215`) emits stage IR files. Stages dumped: `vhlo` (line 432), `shlo` (452), `shlo_frontend` (465), `shlo_compiler` (843), `shlo_set_mesh_attr` (852), `ttir` (880), `ttnn` (1117), `ttnn_runtime` (1336). **Filename pattern is NOT `<stage>.mlir`** — it includes the model name + a timestamp suffix (e.g., `shlo_<model>_<timestamp>.mlir`). All references in the spec to dump files use globbing (`ls <export_path>/shlo_*.mlir` or `find <export_path> -name "shlo_*"`). Trigger via `torch_xla.set_custom_compile_options({"export_path": "/tmp/dump"})`. Use for Phase 0.2 (block-arg attrs in `shlo`-stage file) and Phase 0.3 (`to_layout`-pair count in `ttnn`-stage file).
...
| 0.2 | Dump tt-xla StableHLO + TTIR for one decode step. Inspect block-arg attrs | Set `torch_xla.set_custom_compile_options({"export_path": "/tmp/shlo_dump", "enable_const_eval": True})` before warm-up. Run probe with `--decode 1`. **First confirm the round-trip works**: after one decode, `ls /tmp/shlo_dump/shlo_*.mlir` must list at least one file (note: filenames include model + timestamp; use a glob). If empty, the Python→C++ option-key serialization isn't reaching `module_builder.cc` — investigate before any other Phase 0 work. Then grep the matched `shlo_*.mlir` file for `mhlo.`/`xla.`/`tf.`/`jax.`/`torch.`/`_xla` -prefixed arg attrs. | Confirmed torch_xla marker name(s) on block args, OR "no per-arg marker" | "No per-arg marker" → Strategy C (heuristic) handles via Phase 4 auto-detect. NOT a descope. |
| 0.3 | Count `to_layout`-pair patterns in TTNN IR | Open the `ttnn`-stage MLIR file dumped under `<export_path>/` from 0.2 (filename pattern is `ttnn_<model>_<timestamp>.mlir`, NOT `ttnn.mlir`; glob to find it). Count occurrences of `ttnn.to_layout(<allowlist_op>(ttnn.to_layout(...)))` triples — i.e., a `to_layout` whose result feeds an op in the Phase-3 allowlist whose result feeds another `to_layout` with the original layout restored. Use a small Python script that parses the MLIR text-form. **Note:** `ttnn.tilize`/`ttnn.untilize` are NOT MLIR ops (review 6 finding) — only `ttnn.to_layout` survives in IR. Don't grep for the runtime kernel names. | Cancellable-triple count | If < 50 per decode step, de-scope A.2.a (Phase 3 → skipped). Document count in Phase 5. |
...
1. Edit `/home/mhnie/tt-xla/third_party/CMakeLists.txt`, ExternalProject_Add block at lines 47–85:
   ...
   - Clear the hardcoded path string at line 38 (the value of `WITH_METAL_RUNTIME_ROOT_SET`) to an empty string — do NOT remove lines 36–39 wholesale because the variable is also referenced at lines 58 and 89 (BUILD_COMMAND and explorer step). Since sub-bullet "set BUILD_COMMAND \"\"" overrides line 58 and the explorer step (line 89) only fires when `TTXLA_ENABLE_EXPLORER` is set, an empty value is safe. Caller (build_and_install.sh, see step 3) provides the actual `TT_METAL_RUNTIME_ROOT` via env var.
...
2. Assign `stablehlo_pipeline_options.argumentTypeMap = classifyArgs(mlir_module);` immediately AFTER `stablehlo_pipeline_options` is constructed at `module_builder.cc:831` (i.e., between line 831 and the call to `createStableHLOPipeline(...)` that consumes the options). Insertion line is approximately 832.
