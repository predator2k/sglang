...
| `set_custom_compile_options` Python-bool serialization mismatch (`True` vs `"true"`) for `enable_const_eval` AND `enable_const_eval_on_cpu` | Phase 0.2 sub-step: verify the round-trip by inspecting the parsed value via the existing INFO log line in `compile_options.cc:44–49` (same parseBoolOption at lines 47–49 for `enable_const_eval_on_cpu`); if `parseBoolOption` rejects `"True"`, set the option value as a string `"true"`/`"false"` explicitly from Python. |
