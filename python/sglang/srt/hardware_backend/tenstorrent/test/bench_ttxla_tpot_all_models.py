"""Benchmark TPOT for all tt-xla models. 2K input, 20 decode tokens, 2 runs."""
...
# Gated models (Llama, Gemma) need HF_TOKEN — skip if not set
MODELS = [
...
for model_id in MODELS:
    ...
    try:
        ...
        r["tpot_min_ms"] = round(min(steady), 1)
        r["tpot_max_ms"] = round(max(steady), 1)
        r["tok_s"] = round(1000 / r["tpot_ms"], 1)
        r["jit_decode_ms"] = [round(t, 0) for t in decode_times[:SKIP_JIT]]
        r["status"] = "PASS"
        print(f"  TPOT: {r['tpot_ms']}ms ({r['tok_s']} tok/s)  jit={r['jit_decode_ms']}", flush=True)
...
print(f"\n{'='*80}")
print(f"{'Model':<22} {'Arch':<22} {'H':>5} {'L':>3} {'TTFT':>7} {'TPOT':>7} {'tok/s':>6} {'St':>4}")
print(f"{'-'*22} {'-'*22} {'-'*5} {'-'*3} {'-'*7} {'-'*7} {'-'*6} {'-'*4}")
for r in results:
    print(f"{r['model']:<22} {r.get('arch','?'):<22} "
          f"{r.get('hidden',''):>5} {r.get('layers',''):>3} "
          f"{str(r.get('ttft_warmup_ms','—'))+'ms':>7} "
          f"{str(r.get('tpot_ms','—'))+'ms':>7} "
          f"{r.get('tok_s','—'):>6} "
          f"{r['status']:>4}")

out_path = "/sglang/python/sglang/srt/hardware_backend/tenstorrent/test/_fixtures/v145_ttxla_tpot_all_models.json"
