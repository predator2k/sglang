...
def run_mode(native: bool, n_steps: int = 20):
    ...
    try:
        ...
        for step in range(n_steps):
            ...
            logits = decode_one_step(
                model, args, step=step, token_id=prev,
    ...
    # report
    print(f"\n[{'native' if native else 'host'}] step times (ms):", flush=True)
    for i, t in enumerate(times):
        print(f"  step={i:2d}  {1000*t:.1f} ms", flush=True)
    warm = times[5:]
    if warm:
        avg = sum(warm) / len(warm)
        print(f"\n[{'native' if native else 'host'}] WARM avg (steps 5-{len(times)-1}): {1000*avg:.1f} ms", flush=True)
    return times
